...
function available_payment_list($support_cod, $cod_fee = 0, $is_online = false)
{
    ...
    if(isset($modules))
    {
		//by tiandi 支付方式各平台兼容显示控制
		foreach($modules as $k=>$v) {
			//如果不在微信里，则不显示微信jsapi支付方式
			if($v['pay_code'] == 'wxjsapi' && !is_from_weixin())
				unset($modules[$k]);
			//如果在微信里，则不显示支付宝支付方式
			if($v['pay_code'] == 'alipay' && is_from_weixin())
				unset($modules[$k]);
			//如果在微信里，则不显示微信H5支付方式
			if($v['pay_code'] == 'wxh5' && is_from_weixin())
				unset($modules[$k]);
			//如果在PC里，则不显示微信H5支付方式
			if($v['pay_code'] == 'wxh5' && !is_from_mobile())
				unset($modules[$k]);
		}
        return $modules;
    }
