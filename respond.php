...
// by tiandi微信支付同步返回，无法携带参数，所以默认显示支付成功页面
elseif($pay_code == 'wxh5') {
	if(isset($_GET['out_trade_no'])) {
		...
		//公众号参数
		$sql = "SELECT pay_config FROM " .$GLOBALS['ecs']->table('payment'). " WHERE pay_code = 'wxh5'";
		$result = $GLOBALS['db']->getOne($sql);
		$config = unserialize_config($result);
		$apikey = $config['wxh5_apikey'];
		$appid = $config['wxh5_appid'];
		$secret = $config['wxh5_secret'];
		$mchid = $config['wxh5_mchid'];
		...
		$postxml = $wechat->create_xml($apikey,2); 
		$prepayobj = $wechat->queryorder($postxml);
		
		if(isset($prepayobj->trade_state) && $prepayobj->trade_state == 'SUCCESS') {
			$msg = $_LANG['pay_success'];
		}
		else {
			$msg = '付款失败或者交易记录同步延迟，请稍后再查询。';
		}
	}
	else 
		$msg = $_LANG['pay_success'];
